...
| `NEON_RECOVERY_API_KEY` | Secret | Recovery control only | Issue a project-scoped Neon control-plane key limited to recovery branch lifecycle. It must not be an organization-wide infrastructure key or database credential. Rotate through Neon and the Worker secret store together. |
| `NEON_PROJECT_ID` | Sensitive identifier | Recovery control only | Exact project selected by the project-scoped recovery key. Store with Worker secrets so it cannot be redirected by a request. |
| `NEON_PARENT_BRANCH_ID` | Sensitive identifier | Recovery control only | Exact production parent branch allowed for PITR children. It is independent from serving `NEON_BRANCH_ID`; every create, reconcile, reset, URI, and delete operation rechecks it. |
| `RECOVERY_EVIDENCE_URL` | Secret operational endpoint | Recovery control only | Exact safe HTTPS endpoint of the separate restricted verifier and aggregate observability authority. No query string, redirect, embedded credential, or test implementation is accepted in production. |
| `RECOVERY_EVIDENCE_TOKEN` | Secret | Recovery control and recovery evidence authority | Independent bearer credential for the metadata-only verification request. The request contains branch identity, timings, drill kind, and aggregate replay counts only; it never contains a database URI, object key, tenant identifier, credential, or content. |
